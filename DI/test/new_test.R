...
net_e_1<-makeNetwork(early_ctrl_data,alpha)
net_e_2<-makeNetwork(early_tumor_data,alpha)
gene1<-mergeNetwork(net_e_1,net_e_2)

middle_ctrl_data<-t(testdata[,middle_ctrl])
middle_tumor_data<-t(testdata[,middle_tumor])
net_m_1<-makeNetwork(middle_ctrl_data,alpha)
net_m_2<-makeNetwork(middle_tumor_data,alpha)
gene2<-mergeNetwork(net_m_1,net_m_2)

late_ctrl_data<-t(testdata[,late_ctrl])
late_tumor_data<-t(testdata[,late_tumor])
net_l_1<-makeNetwork(late_ctrl_data,alpha)
net_l_2<-makeNetwork(late_tumor_data,alpha)
